...
;(function ($w) {
    ...
    var DEFAULT_OPTIONS = {
        ...
        support_html: true,
        select_node_callback: null,//节点选中事件回调
        select_clear_callback: null,//节点取消选中事件回调
    ...
    jm.format = {
        ...
        freemind: {
            example: {
                ...
                "format": "freemind",
                "data": "<map version=\"1.0.1\"><node ID=\"root\" TEXT=\"根节点\"/></map>"
            },
            get_mind: function (source) {
    ...
    jm.prototype = {
        ...
        set_mode: function (mode) {
            var mode_old = this.options.mode;
            this.options.mode = (!!mode) ? mode : null;
            if (mode_old != this.options.mode) {
                this.layout.opts.mode = mode;
         ...
         * @param width
         */
        set_line: function (color, width) {
            if (color) {
                this.options.view.line_color = color;
                this.view.opts.line_color = color;
            }
            if (width) {
                this.options.view.line_width = width;
                this.view.opts.line_width = width;
            }
            var json = this.get_data();
            this.show(json);
        },
            ...
            if (node.parent.isroot) {
                ...
                for (var i = 0; i < c.length; i++) {
                    ...
                    if (this.options.mode == 'side') {
                        ...
                        prev = ni;
                    } else if (node.direction === ni.direction) {
                        if (node.id === ni.id) {
                            n = prev;
                        }
                        prev = ni;
        ...
        set_node_font_style: function (nodeid, size, weight, style, family) {
            if (this.get_editable()) {
                var node = this.mind.get_node(nodeid);
                if (!!node) {
                    if (!!size) {
    ...
    jm.view_provider.prototype = {
        ...
        add_drag_event() {
            //添加画布拖动事件
            var dragCanvasEle = document.getElementById(this.jm.options.container);
            //屏蔽此对象的右键菜单
            dragCanvasEle.oncontextmenu = () => false;
            var that = this;
            dragCanvasEle.addEventListener("mousedown", function (event) {
                that._drag_canvas(that, event);
            }, false);
        },

        _drag_canvas: function (that, event) {
            var event = event || window.event;
            if (event.button == 1) {
                return;
            }
            if (event.target.className == 'jsmind-editor') {
                return;
            }
            if (event.button == 0) {
            ...
            var dragCanvasEle = document.getElementById(that.jm.options.container);
            dragCanvasEle.style.cursor = "move";
            var mouseStartX = event.clientX; //开始时鼠标位置
            var mouseStartY = event.clientY; //开始时鼠标位置

            document.onmousemove = function (event) {
                var mouseEndX = event.clientX; //当时鼠标位置
                var mouseEndY = event.clientY; //当时鼠标位置

                var x = that.translate.x + mouseEndX - mouseStartX;
                var y = that.translate.y + mouseEndY - mouseStartY;
                //设置偏移位置
                that.setZoom(that.actualZoom, x, y);
            }
            document.onmouseup = function (event) {
                ...
                dragCanvasEle.style.cursor = "auto";

                var mouseEndX = event.clientX; //结束时鼠标位置
                var mouseEndY = event.clientY; //结束时鼠标位置

                that.translate.x += mouseEndX - mouseStartX;
                that.translate.y += mouseEndY - mouseStartY;
            }
        },
        ...
        select_node: function (node) {
            ...
            if (node && this.jm.options.select_node_callback) {
                this.jm.options.select_node_callback(node);
            }

            if (!node && this.jm.options.select_clear_callback) {
                this.jm.options.select_clear_callback();
            }
        },
        ...
        edit_node_begin: function (node) {
            ...
            this.e_editor.innerText = topic;
            if (window.getSelection) {//ie11 10 9 ff safari
                this.e_editor.focus(); //解决ff不获取焦点无法定位问题
                var range = window.getSelection();//创建range
                range.selectAllChildren(this.e_editor);//range 选择obj下所有子内容
                range.collapseToEnd();//光标移至最后
            } else if (document.selection) {//ie10 9 8 7 6 5
                var range = document.selection.createRange();//创建选择对象
                //var range = document.body.createTextRange();
                range.moveToElementText(this.e_editor);//range定位到obj
        ...
        edit_node_end: function () {
            if (this.editing_node != null) {
                ...
                var topic = this.e_editor.innerText;
        ...
        zoomIn: function () {
            return this.setZoom(this.actualZoom + this.zoomStep, this.translate.x, this.translate.y);
        },
        ...
        setZoom: function (zoom, translate_x, translate_y) {
            zoom = zoom.toFixed(1);
            if ((zoom < this.minZoom) || (zoom > this.maxZoom)) {
                return false;
            }
            this.actualZoom = parseFloat(zoom);
            for (var i = 0; i < this.e_panel.children.length; i++) {
                this.e_panel.children[i].style.transform = 'scale(' + zoom + ') translate(' + translate_x + 'px, ' + translate_y + 'px)';
            }

            //$('canvas.jsmind').css("transform", 'translate(' + x + 'px, ' + y + 'px)');
            //$('jmnodes').css("transform", 'translate(' + x + 'px, ' + y + 'px)');
    ...
    jm.shortcut_provider.prototype = {
        ...
        _handle_direction: function (_jm, e, d) {
            ...
            var node = null;
            if (!!selected_node) {
                if (_jm.options.mode == 'side' && selected_node.direction == jm.direction.left) {
                    if (d == jm.direction.right) {
                        d = jm.direction.left;
                    } else {
                ...
                if (selected_node.isroot) {
                    ...
                    var children = [];
                    for (var i = 0; i < c.length; i++) {
                        if (_jm.options.mode == 'side') {
                            children.push(i);
                        } else if (c[i].direction === d) {
                            children.push(i);
                        }
                    }
